...
export async function generateGeminiContent(
  ...
  try {
    ...
    let activeModelName = options.modelName;
    ...
    if (options.useThinkingMode && !activeModelName) {
      ...
      console.warn(
        `L'option 'useThinkingMode' est activée. Utilisation du modèle : ${activeModelName}. ` +
    ...
    const model = genAI.getGenerativeModel({
      ...
      safetySettings: [ // Paramètres de sécurité recommandés
    ...
    const generationConfig: GenerationConfig = {
      temperature: options.temperature ?? 0.7, // Température par défaut équilibrée
      topK: options.topK,
      topP: options.topP,
      maxOutputTokens: options.maxOutputTokens ?? 2048, // Limite de tokens par défaut
    };

    // Construction des "parts" pour le message actuel de l'utilisateur
    const currentSdkParts: Part[] = [];
    const itemsArray = Array.isArray(promptItems) ? promptItems : [promptItems];

    for (const item of itemsArray) {
      if (typeof item === "string") {
        currentSdkParts.push({ text: item });
      } else { // C'est une ImagePart
        currentSdkParts.push({
          inlineData: {
    ...
    if (currentSdkParts.length === 0) {
      throw new Error("Le prompt ne peut pas être vide.");
    }

    // Construction de la structure `contents` complète pour l'API
    // Commence avec l'historique, s'il existe
    const requestContents: Content[] = options.history ? [...options.history] : [];
    // Ajoute le message actuel de l'utilisateur
    requestContents.push({ role: "user", parts: currentSdkParts });

    const result = await model.generateContent({
      contents: requestContents, // Utilise les contents construits avec l'historique
      generationConfig,
    });
    ...
    if (error instanceof Error) {
      ...
      if (error.message.toLowerCase().includes("permission denied") || error.message.includes("access to model")) {
        throw new Error(`Accès refusé. Vérifiez que votre clé API a la permission d'utiliser le modèle spécifié (${options.modelName || "par défaut"}).`);
      }
 ...
 * @returns Une promesse qui se résout avec le score (0-4) et la réponse brute, ou null si échec.
 */
export async function analyzeImageCondition(
  image: ImagePart,
  categorySpecificPrompt: string,
  options: GeminiOptions = {}
): Promise<{ score: number | null; rawResponse: string } | null> {
  if (!categorySpecificPrompt || !categorySpecificPrompt.trim()) {
    console.warn("Le prompt spécifique à la catégorie est requis pour l'analyse d'image.");
    return null;
  }

  // S'assurer que le prompt demande bien un score entre 0 et 4
  const fullPrompt = `${categorySpecificPrompt}. Répondez uniquement avec un chiffre entre 0 (très mauvais état / inutilisable) et 4 (excellent état / comme neuf). Votre réponse doit être uniquement ce chiffre.`;
  
  const promptItems: PromptItem[] = [
    fullPrompt,
    image, // L'objet ImagePart directement
  ];

  try {
    // Utiliser un modèle capable de vision. gemini-1.5-flash-latest est un bon choix par défaut.
    const modelName = options.modelName || 'gemini-1.5-flash-latest';
    const geminiOptions: GeminiOptions = {
      ...options,
      modelName: modelName,
      maxOutputTokens: options.maxOutputTokens ?? 10, // Réponse courte attendue (juste un chiffre)
      temperature: options.temperature ?? 0.2, // Faible température pour une réponse plus déterministe
    };
    ...
    // Retourner null ou une structure d'erreur spécifique si nécessaire
    // Pour l'instant, on retourne la réponse brute même en cas d'erreur d'extraction de score,
    // mais on pourrait vouloir la réponse brute uniquement si l'appel API a réussi.
    // Si generateGeminiContent lève une exception, elle sera propagée par défaut.
    // Ici, on pourrait vouloir capturer l'erreur de generateGeminiContent et retourner null.
    if (error instanceof Error) {
      return { score: null, rawResponse: `Erreur Gemini: ${error.message}` };
    }
    return { score: null, rawResponse: "Erreur inconnue lors de l'analyse Gemini." };
  }
}
